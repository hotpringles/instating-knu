...
module.exports = (upload) => {
   ...
   * @access  Public
   */
  router.get("/matching-cards", async (req, res) => {
    try {
   ...
   * @access  Private
   */
  router.post("/matching-cards", auth, async (req, res) => {
    ...
    try {
      ...
      if (existingCard) {
        return res.status(409).json({
          message:
            "이미 등록한 매칭 카드가 있습니다. 하나만 등록할 수 있습니다.",
        });
      }
   ...
   * @access  Private
   */
  router.post("/cards/:cardId/reveal", auth, async (req, res) => {
    const { cardId } = req.params;
   ...
   * @access  Private
   */
  router.delete("/matching-cards", auth, async (req, res) => {
    const userId = req.user.id;
    try {
   ...
   * @access  Admin
   */
  router.get("/admin/cards", auth, adminAuth, async (req, res) => {
    try {
      const cards = await prisma.matchingCard.findMany({
        orderBy: { createdAt: "desc" },
        include: {
   ...
   * @access  Admin
   */
  router.delete("/admin/cards/:cardId", auth, adminAuth, async (req, res) => {
    const { cardId } = req.params;
    try {
   ...
   * @access  Admin
   */
  router.delete("/admin/users/:userId", auth, adminAuth, async (req, res) => {
    ...
    try {
      ...
      if (userToDelete.photo) {
        const filename = path.basename(userToDelete.photo);
